    ...
    return (
      <div className="container mx-auto max-w-4xl py-8">
        <Card className="bg-[#0f172a] border-[#1e293b]">
          <CardHeader>
            <CardTitle className="text-xl font-bold text-white">Inicia sesión para gestionar tus
              amigos</CardTitle>
          ...
          <Card className="md:col-span-2 bg-[#0f172a] border-[#1e293b]">
            <CardHeader className="pb-3">
              <div className="flex items-center">
                ...
                <CardTitle className="text-lg font-semibold text-white">Amigos
                  ({filteredFriends.length})</CardTitle>
              </div>
              <CardDescription className="text-gray-400">
              ...
              <div className="space-y-4">
                <div>
                  <h3 className="text-sm font-medium text-white mb-2 flex items-center">
                    <span
                      className="inline-flex items-center justify-center h-5 w-5 rounded-full bg-blue-500/20 text-blue-400 text-xs mr-2">
                  ...
                  {incomingRequests.length === 0 ? (
                    ...
                    <div className="space-y-2">
                      {incomingRequests.map((request) => (
                        <Card key={`${request.requesterUsername}-${request.createdAt}`}
                              className="bg-[#1e293b]/30 border-[#1e293b]">
                          <CardContent className="p-3">
                            <div className="flex items-center justify-between mb-2">
                              <div className="flex items-center">
                                <Avatar
                                  className="h-8 w-8 mr-2 border border-[#1e293b]">
                                  <AvatarImage
                                    src={`https://avatar.vercel.sh/${request.requesterUsername}`}/>
                                  <AvatarFallback
                                    className="bg-[#1e293b] text-blue-400">
                                    {request.requesterUsername.charAt(0).toUpperCase()}
                                  </AvatarFallback>
                                </Avatar>
                                <span
                                  className="text-sm text-white">{request.requesterUsername}</span>
                              </div>
                        ...
                        >
                          <div className="flex items-center">
                            <Avatar className="h-6 w-6 mr-2 border border-[#1e293b]">
                              <AvatarImage
                                src={`https://avatar.vercel.sh/${request.recipientUsername}`}/>
                              <AvatarFallback
                                className="bg-[#1e293b] text-blue-400 text-xs">
                                {request.recipientUsername.charAt(0).toUpperCase()}
                              </AvatarFallback>
                            </Avatar>
                            <span
                              className="text-sm text-white">{request.recipientUsername}</span>
                          </div>
                          <span
