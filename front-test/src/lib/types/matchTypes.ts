...
interface InfoDto {
    ...
    gameCreation: number;         // Unix timestamp (long)
    gameDuration: number;         // tiempo en milisegundos o segundos según la versión
    gameEndTimestamp?: number;    // Unix timestamp (long), opcional porque puede no venir
    gameId: number;               // long
    gameMode: string;
    gameName: string;
    gameStartTimestamp: number;   // Unix timestamp (long)
    gameType: string;
    gameVersion: string;
    mapId: number;
    ...
    queueId: number;
    teams: TeamDto[];
    tournamentCode?: string;      // opcional porque puede no venir
}
